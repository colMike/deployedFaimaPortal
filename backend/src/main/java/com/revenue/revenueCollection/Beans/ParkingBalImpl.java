...
import com.revenue.revenueCollection.Dao.ParkingDao;
import com.revenue.revenueCollection.Dao.carTypeDao;
import com.revenue.revenueCollection.Dao.priceConfigDao;
import com.revenue.revenueCollection.Interfaces.ParkingBal;
import com.revenue.revenueCollection.Models.*;
import com.revenue.revenueCollection._exceptions.GenericServiceException;
...
public class ParkingBalImpl implements ParkingBal {
  ...
  @Autowired priceConfigDao priceDao;

  @Autowired carTypeDao carDao;

  @Override
  public ResponseMessage RegisterParking(Parking service) {

    System.out.println("This is the updated parking");
    System.out.println(service);

    Parking retrievedParking = parkingDao.findByServiceName(service.serviceName);
    long numOfRecords = parkingDao.count();
    numOfRecords = numOfRecords+1;

    if (retrievedParking != null)
      return new ResponseMessage(201, "Parking creation failed. Parking already exists");
    ...
    try {
      ...
      if (service != null) {
        ...
        service.setServiceCode("PARKING"+numOfRecords++);
  ...
  public ResponseMessage updateParking(Parking service) {
    ...
    try {
      ...
      if (service.getServiceId() != 0) {
        Parking service1 =
            parkingDao.findById(service.serviceId).orElseThrow(NullPointerException::new);
...
//        service1.setServiceCode(service.getServiceCode());
  ...
  //    @Override
  //    public Parking GetAllParkingByParkingName(String serviceName){
  //        return parkingDao.findByServiceName(serviceName);
  //    }
  ...
  public ResponseMessage updatePriceConfig(priceConfig priceConfig) {
    try {
      ...
      if (priceConfig.id != 0) {
        priceConfig service1 =
            priceDao.findById(priceConfig.id).orElseThrow(NullPointerException::new);
  ...
  public ResponseMessage RegisterCarType(CarType carType) {
    ...
    if (carType != null) {
      ...
      if (carDao.findByCarTypeName(carType.carTypeName) != null) {
        System.out.println("A car with this name exists");
        throw new GenericServiceException("Record already exists");
      }
  ...
  public ResponseMessage updateCarType(CarType carType) {
    try {
      ...
      if (carType.carTypeId != 0) {
        CarType service1 =
            carDao.findById(carType.carTypeId).orElseThrow(NullPointerException::new);
