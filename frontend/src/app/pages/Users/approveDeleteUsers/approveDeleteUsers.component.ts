...
@Component({
  selector: 'app-users',
  templateUrl: './approveDeleteUsers.component.html'

})
export class approveDeleteUsersComponent implements OnInit {
 ...
 */
  ...
  Reject(){
    ...
    for (var i = 0; i <= this.users1.length - 1; i++) {
      if (this.users1[i].deletedBy === this.users1.approvedBy) {
    ...
    this.userSvc.approveDeleteUser(user2).subscribe((response) => {
      this.response = response;
      console.log(this.response.status,"response")
      if (this.response.status===200) {
  ...
  getUsers(){
    ...
    this.userSvc.getUsersToApproveDelete().subscribe(users =>{
      // if(data){
      this.users1 = users;
      //this.blockUI.stop();
      /* }
