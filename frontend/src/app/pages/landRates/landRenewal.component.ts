...
@Component({
  templateUrl: './landRenewal.html',


})
export class LandRenewalComponent implements OnInit, OnDestroy {
  groups: any;
  types: any;
 ...
 */
  ...
  getAllLandToRenewToRenew() {
    this.blockUI.start('please wait');
    this.appEditMode=false;
    this.Landervice.gtLandToRenew().subscribe(data => {
  ...
  updateLand(){
    ...
    this.Landervice.renewLand(Land2).subscribe(res => {
      this.res = res;
      console.log(this.res,"LLLLLLLLLLLLLLLL")
      if (this.res.status === 200) {
        this.appEditMode = false;
