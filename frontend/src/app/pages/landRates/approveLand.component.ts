...
@Component({
  templateUrl: './approveLand.component.html',


})
export class ApproveLandComponent implements OnInit, OnDestroy {
  groups: any;
  types: any;
  ...
  constructor(private landSvc: LandService,private regionSvc: RegionService,public formBuilder: FormBuilder, private toastr: ToastrService,  private modalService: NgbModal) {
 ...
 */
  ...
  Approve(){
    //this.land1.permitId=this.permits.permitId;
    // this.land1.permitId = this.landForm.get('land1').value;

    //this.land1.permitId=this.permitId;


    console.log( this.land1.permitId, " this.land1.permitId")
    this.land1.approved = 'V';

    const session=localStorage.getItem('currentUser');

    this.sessionId=JSON.parse(session);

    console.log(this.sessionId.entity.userId,"this.land1");
    console.log(this.sessionId.entity,"this.land1");

    this.land1.approvedBy=this.sessionId.entity.userId;


    for (var i = 0; i <= this.land1.length - 1; i++) {
      console.log(this.land1[i].createdBy, "permits @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@2")

      if (this.land1[i].createdBy === this.land1.approvedBy) {

        return this.toastr.warning('Land rate creator cannot Approve a land rate he/she created', ' Warning!', {timeOut: 3000});
    ...
    this.landSvc.approveLand(permit2).subscribe((response) => {
      this.blockUI.stop();
  ...
  Reject(){
    ...
    console.log( this.land1.id, " this.land1.permitId")
    this.land1.approved = 'R';
    ...
    console.log(this.land1.createdBy, "permits @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@2")
    for (var i = 0; i <= this.land1.length - 1; i++) {
  ...
  getAllLand() {
    this.blockUI.start('Please wait...');
    this.appEditMode=false;
    this.landSvc.getLandToApprove().subscribe(data => {
      this.blockUI.stop();
  ...
  getAllpermitTypes() {
    ...
    this.landSvc.gtPermitType().subscribe(data => {
  ...
  addLand() {
    ...
    this.landSvc.addLand(this.land1).subscribe(res => {
      this.res = res;
  ...
  updateLand(){
    ...
    this.land1.nationalIdNumber = this.landForm.get('nationalIdNumber').value;
    this.land1.wardId= this.landForm.get('wardId').value;
    this.land1.subcCountyId= this.landForm.get('subcountyId').value;
    this.land1.id= this.landForm.get('id').value;
    ...
    this.landSvc.addLand(Land2).subscribe(res => {
      this.res = res;
      console.log(this.res,"LLLLLLLLLLLLLLLL")
      if (this.res.status === 200) {
        this.appEditMode = false;
