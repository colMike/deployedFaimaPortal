  ...
  updateParking() {
    ...
    this.parkingSvc.updateParkings(Parking2).subscribe((response) => {
      ...
      if (this.response.status === 200) {
        ...
        // alert(response.respMessage);
        console.log('this.response.context.entity.respCode');
        console.log(this.response.context.entity.respCode);

        if (this.response.context.entity.respCode === 200) {
          return this.toastr.success('Great! The Parking information was saved successfully."', ' Success!', {timeOut: 3000});
        }
        return this.toastr.success('A parking with a similar name already exists."', ' Success!', {timeOut: 3000});
