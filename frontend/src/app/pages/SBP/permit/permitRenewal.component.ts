...
@Component({
  templateUrl: './permitRenewal.component.html',
  styleUrls: ['./permit.component.css']

})
export class permitRenewalComponent implements OnInit, OnDestroy {
  groups: any;
  types: any;
 ...
 */
  ...
  getAllPermitsToRenew() {
    this.blockUI.start('please wait');

    this.appEditMode=false;
    this.permitService.gtPermitToRenew().subscribe(data => {
  ...
  addPermit() {
    if (this.permit1.regNo === null) {
     return  this.toastr.warning('Please specify the businessName', 'Alert!', { timeOut: 4000 });
    }
    ...
    this.blockUI.start('please wait');
    this.permitService.renewPermit(this.permit1).subscribe(res => {
      this.res = res;
      this.blockUI.stop();
  ...
  updatePermit(){
    //this.initAddPermit();
     //this.permit1 = this.permitForm.value;
 /*   if (this.permit1.permitId === null) {
      return  this.toastr.warning('Please specify the permit number', 'Alert!', { timeOut: 4000 });
    }*/
    console.log(this.permit1.permitId,"when updating")
    //this.permit1.permitId= this.permitForm.get('permitId').value;


    this.permit1.status='RW';
    this.permit1.app_type='Renew';
    ...
    const permit2 ={
      ...
      'status':this.permit1.status,
      'appType':this.permit1.appType,
    ...
    this.permitService.renewPermit(permit2).subscribe(res => {
      this.res = res;
      console.log(this.res,"LLLLLLLLLLLLLLLL")
      if (this.res.status === 200) {
        this.appEditMode = false;
        ...
        this.modalService.dismissAll();
        this.blockUI.stop();
        return this.toastr.success(res.message, 'Success!', { timeOut: 4000 });
      }
      if (this.res.status === true) {
        // if (this.userGroup.id === 0) {
        //     this.log(this.rightId, 'added group ' + this.userGroup.groupName);
