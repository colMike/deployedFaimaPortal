...
@Component({
  templateUrl: './approveDeleteRole.component.html'

})
export class approveDeleteRoleComponent implements OnInit, OnDestroy {
  groups: any;
  types: any;
  res: any;
  group1: any;
  ...
  Approve() {
    ...
    for (var i = 0; i <= this.group1.length - 1; i++) {
      ...
      if (this.group1[i].deletedBy === this.group1.approvedBy) {

        return this.toastr.warning('A user who deletes a Role cannot approve the deletion', ' Warning!', {timeOut: 3000});
    ...
    const user2 = {
      'id': this.group1.id,
      'deleted': this.group1.deleted


    };
    console.log(user2, '$$$$$$$$$$$$$$$');
    console.log(this.group1, '$$$$$$$$$$$$$$$$');
    this.apiService.approveDeleteRole(user2).subscribe((response) => {
      ...
      if (this.response.status === 200) {
        ...
        this.getGroupsAndUserRights();
        this.editMode = true;
        return this.toastr.success('Great! The Role information was approved successfully', ' Warning!', {timeOut: 3000});
  ...
  Reject() {
    ...
    for (var i = 0; i <= this.group1.length - 1; i++) {
      ...
      if (this.group1[i].deletedBy === this.group1.approvedBy) {
        ...
        return this.toastr.warning('A user who deletes a Role cannot Reject the deletion', ' Warning!', {timeOut: 3000});
    ...
    this.apiService.approveDeleteRole(user2).subscribe((response) => {
      ...
      if (this.response.status === 200) {
        ...
        this.getGroupsAndUserRights();
        this.editMode = true;
        return this.toastr.success('Great! The Role information was rejected successfully', ' Warning!', {timeOut: 3000});
  ...
  getGroupsAndUserRights() {
    ...
    this.apiService.getRolesToApproveDelete().subscribe(data => {
